...
%         D_o = (veh_tar.R_veh+ob_dist)^2*ones(veh_tar.Np,1);
        ...
        AA = diag(ones(1,Np-1),1)+diag(-ones(1,Np));
        f{1,i}.A_o = [f{1,i}.A_o;AA;-AA;veh_tar.B_tilde; -veh_tar.B_tilde];
        f{1,i}.b_o = -D_o+Phi*veh_tar.A_tilde*[veh_tar.x_now;veh_tar.y_now;veh_tar.theta_now]+Phi*veh_tar.W_tilde;        
        BB = 0.05*ones(Np,1);
        UM_bar = veh_tar.UM-veh_tar.W_tilde-veh_tar.A_tilde*[veh_tar.x_now;veh_tar.y_now;veh_tar.theta_now];
        LM_bar = veh_tar.LM-veh_tar.W_tilde-veh_tar.A_tilde*[veh_tar.x_now;veh_tar.y_now;veh_tar.theta_now];
        f{1,i}.b_o = [f{1,i}.b_o;BB;BB;UM_bar;-LM_bar];
