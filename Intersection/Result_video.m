...
%         plot(linspace(17/ratio, 17/ratio, 5), linspace(0, Env_con.height/ratio, 5),'--','color',[hex2dec('6c')/255, hex2dec('7b')/255, hex2dec('8b')/255],'linewidth',1.5);
        ...
        for i = 1:n
            if tt <= tail
                ...
                for ttt = 1:2:tt
                    plot(veh_cell{1,i}.real_path(ttt,1)/ratio, veh_cell{1,i}.real_path(ttt,2)/ratio,'marker', marker(i),'markeredgecolor', tail_color(ttt,:),'markerfacecolor', tail_color(ttt,:),'markersize',10);
                end
            else
                ...
                for ttt = tt-tail+1:2:tt
                    plot(veh_cell{1,i}.real_path(ttt,1)/ratio, veh_cell{1,i}.real_path(ttt,2)/ratio,'marker', marker(i),'markeredgecolor', tail_color(ttt-tt+tail,:),'markerfacecolor', tail_color(tail-tt+ttt,:),'markersize',10);
                end
